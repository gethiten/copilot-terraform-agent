"""
Copilot Terraform Agent - Azure OpenAI Backend
This app provides API endpoints for Microsoft Copilot Studio to generate Terraform code.
Uses Azure OpenAI directly (no Foundry dependency).
"""
...
def parse_terraform_blocks(response_text: str) -> dict:
    ...
    if not matches:
        ...
        blocks['main.tf'] = response_text.strip()
        return blocks
...
def copilot_generate():
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print("🤖 Copilot Terraform Generation Request")
        print(f"{'='*60}")
        print(f"Prompt: {prompt}")
        print(f"Location: {location}")
        print(f"Resource Group: {resource_group_name or '(auto)'}")
        print(f"{'='*60}\n")
...
if __name__ == '__main__':
    ...
    port = int(os.getenv('FLASK_PORT', 5001))  # Different port from Foundry app
    debug = os.getenv('FLASK_DEBUG', 'True').lower() == 'true'
    
    print("="*60)
    print("🤖 Copilot Terraform Agent")
    print("="*60)
    print(f"Azure OpenAI: {'✅ Configured' if client else '❌ Not configured'}")
